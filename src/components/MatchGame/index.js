...
class MatchGame extends Component {
  ...
  setTimer = () => setInterval(this.decrementTimeElapsedInSec, 1000)

  clearTimer = () => clearInterval(this.timerId)

  decrementTimeElapsedInSec = () => {
  ...
  clickImage = imageUrl => {
    ...
    const isImageClickedMatched = activeImage === imageUrl
    const imageUrlsList = imagesList.map(eachItem => eachItem.imageUrl)
    const randomImage =
      imageUrlsList[Math.floor(Math.random() * imageUrlsList.length)]

    if (isImageClickedMatched) {
      this.setState(prevState => ({
  ...
  renderImagesList = () => {
    ...
    return (
      <div className="match-image-and-images-list-container">
    ...
    return (
      <div className="app-container">
        ...
        <div className="match-game-body">
          {isGameInProgress ? this.renderImagesList() : this.renderScoreCard()}
        </div>
      </div>
    )
  }
